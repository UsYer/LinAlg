...
TEST(PreallocateMemory)
{
 ...
 CHECK_EQUAL(400u, m.size());
 CHECK(!m.empty());
}
TEST(DefaultConstruct)
{
 LinAlg::Matrix<int> m;
 CHECK_EQUAL(0u, m.size());
 CHECK(m.empty());
}
TEST(ConstructWithInitializerList)
{
    LinAlg::Matrix<int> m = {{1, 2, 3, 4}, {5, 6, 7, 8}};
    CHECK_EQUAL(8u, m.size());
    CHECK_EQUAL(2u, m.col_size());
    CHECK_EQUAL(4u, m.row_size());
}
TEST(ConstructFilled)
{
    LinAlg::Matrix<int> m(2,2,13);
    CHECK_EQUAL(4u, m.size());
    CHECK_EQUAL(2u, m.col_size());
    CHECK_EQUAL(2u, m.row_size());
    CHECK_EQUAL(13, m[0]);
    CHECK_EQUAL(13, m[1]);
    CHECK_EQUAL(13, m[2]);
    CHECK_EQUAL(13, m[3]);
...
TEST(VerfiyNoOfRows)
{
    ...
    CHECK_EQUAL(20u, m.row_size());
}

TEST(VerfiyNoOfCols)
{
    LinAlg::Matrix<int> m(20,10);
    CHECK_EQUAL(10u, m.col_size());
}
